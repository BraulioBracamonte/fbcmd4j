...
public class Main {
  ...
  public static void main(String[] args) throws FacebookException, IOException {
    ...
    while(opt != 0 ){
      ...
      switch (opt){
        case 1:
          System.out.println("Descargando los Posts");
          logger.log(Level.INFO, "Descargando los posts" );
          ResponseList objs = facebook.getFeed();
          System.out.println("¿Deseas guardar los resultados?\n1-> Si\n2-> No\n");
          if(getUsrIn(2)==1){
            System.out.println("Guardando los posts");
            List<String> lineas = new ArrayList<>();
            objs.forEach((p)->lineas.add(p.toString()) );
            Path ruta = Paths.get(Paths.get("") + "posts.txt");
            Files.write(ruta, lineas, Charset.forName("UTF-8"));
            logger.log(Level.INFO, "se guardaron los posts en \"posts.txt\"" );
          }
